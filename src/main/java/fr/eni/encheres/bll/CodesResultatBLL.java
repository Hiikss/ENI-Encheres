package fr.eni.encheres.bll;

public class CodesResultatBLL {
	
	public static final int REGLE_EMAIL_ERREUR=20000;
	
	public static final int REGLE_MOT_DE_PASSE_ERREUR=20001;
	
	public static final int REGLE_PSEUDO_ERREUR=20002;
	
	public static final int REGLE_USER_NOM_ERREUR=20003;
	
	public static final int REGLE_USER_PRENOM_ERREUR=20004;
	
	public static final int REGLE_USER_RUE_ERREUR=20005;
	
	public static final int REGLE_USER_CODEPOSTAL_ERREUR=20006;
	
	public static final int REGLE_USER_VILLE_ERREUR=20007;
	
	public static final int REGLE_USER_TELEPHONE_ERREUR=20008;
	
	public static final int MDP_COMPARAISON_ERREUR=20009;

}
